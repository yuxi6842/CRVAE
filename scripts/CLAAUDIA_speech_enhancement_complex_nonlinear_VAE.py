...
output_path = r'/saved/output.mat'
...
# noise_types = ['bbl', 'caf', 'ssn', 'str', 'ped', 'bus']
# noise_types = ['bbl']
...
class SECRVAE(object):
    ...
    def _build_encoder(self, inputs, reuse=False):
        with tf.variable_scope("enc", reuse=reuse):
            enc1 = ComplexDense(units=self._model_conf['cmplx_GRU_z_layer1'], name='enc_layer1', dtype=_dtype)(inputs)
            Relu_enc1 = mod_relu(enc1)
            z_para = ComplexDense(units=self._model_conf['n_latent'] * 3, name='z_para', dtype=_dtype)(Relu_enc1)
    ...
    def _build_decoder(self, z, reuse=False):
        ...
        with tf.variable_scope("dec", reuse=reuse):
            dec1 = ComplexDense(units=self._model_conf['cmplx_GRU_dec1'], name='dec_layer1', dtype=_dtype)(z)
            Relu_dec1 = mod_relu(dec1)
            tempx = ComplexDense(units=self._model_conf['cmplx_fc_dec2'], name='tempx', dtype=_dtype)(Relu_dec1)
            print('Dec: tempx ', tempx)
            tempx = tf.expand_dims(tempx, axis=1)
            x.append(tempx)
            x = tf.concat(x, axis=1, name="recon_x")
        mean_x = tf.reduce_mean(tf.math.abs(x))
