...
def sqrt_init(shape, dtype=None):
    value = (1. / np.sqrt(2.)) * tf.ones(shape)
...
def complex_standardization(input_centred, Vrr, Vii, Vri,
    ...
    input_dim = K.shape(input_centred)[axis]
    ...
    # upper line is the original code, but based on wikipedia, it should be the following:
    t = tf.math.sqrt(tau + s)
    ...
    r_output = broadcast_Wrr * tf.math.real(input_centred) + broadcast_Wri * tf.math.imag(input_centred)
    i_output = broadcast_Wri * tf.math.real(input_centred) + broadcast_Wii * tf.math.imag(input_centred)
...
def ComplexBN(input_centred, Vrr, Vii, Vri, beta,
    ...
    ndim = K.ndim(input_centred)
    input_dim = K.shape(input_centred)[axis]
    print('input_centred ', input_centred)
    print('ndim ', ndim)
    if scale:
    ...
    if scale:
        ...
        print('*-'*20)
        print('gamma_broadcast_shape ', gamma_broadcast_shape)
        print('gamma_rr ', gamma_rr)
        print('gamma_ri ', gamma_ri)
        print('gamma_ii ', gamma_ii)
        ...
        r_output = broadcast_gamma_rr * tf.math.real(standardized_output) + broadcast_gamma_ri * tf.math.imag(standardized_output)
        i_output = broadcast_gamma_ri * tf.math.real(standardized_output) + broadcast_gamma_ii * tf.math.imag(standardized_output)



        if center:
            broadcast_beta = K.reshape(beta, broadcast_beta_shape)
            real_broadcast_beta = broadcast_beta[:, :input_dim]
            imag_broadcast_beta = broadcast_beta[:, input_dim:]
            r_output = r_output + real_broadcast_beta
            i_output = i_output + imag_broadcast_beta
            return tf.complex(r_output, i_output)
        else:
            return tf.complex(r_output, i_output)
    else:
        if center:
            ...
            real_broadcast_beta = broadcast_beta[:, :input_dim]
            imag_broadcast_beta = broadcast_beta[:, input_dim:]
            r_output = tf.math.real(input_centred) + real_broadcast_beta
            i_output = tf.math.imag(input_centred) + imag_broadcast_beta
            return tf.complex(r_output, i_output)
        else:
...
class ComplexBatchNormalization(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.center:
            self.beta = self.add_weight(shape=(input_shape[self.axis] * 2,),
                                        name='beta',
    ...
    def call(self, inputs, training=None):
        ...
        centred_real = tf.math.real(input_centred)
        centred_imag = tf.math.imag(input_centred)
        centred_squared_real = centred_real ** 2
        centred_squared_imag = centred_imag ** 2
