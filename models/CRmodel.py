...
class SECRVAE(object):
    ...
    def _build_encoder(self, inputs, reuse=False):
        ...
        num_units = [self._model_conf['cmplx_GRU_z_layer1'], 512*3]
        z_mu, z_r, z_s, z = [], [], [], []
        with tf.variable_scope("enc", reuse=reuse):
            encoder_cells = [SGRU(units=unit, stiefel=stiefel, reuse=reuse,
                                  dtype=_dtype, dropout=False) for unit in num_units]
    ...
    def _build_decoder(self, z, reuse=False):
        ...
        batch_size = input_shape[0][0]
        num_units = [self._model_conf['cmplx_GRU_z_layer1'], 200]

        with tf.variable_scope("dec", reuse=reuse):
            decoder_cells = [SGRU(units=unit, stiefel=stiefel, reuse=reuse,
                                  dtype=_dtype, dropout=False) for unit in num_units]
    ...
    def _build_model(self):
        ...
        with tf.name_scope("costs"):
            ...
            with tf.name_scope("kld_z"):
                kld_z = tf.math.reduce_mean(cmplx_kld(*qz_x), axis=(1, 2))
